...
const double kpwall = 5;
const double kiwall = 0.0;
const double kdwall = 0.5;
...
const float OBJECT_THRESHOLD = 12.0;  // Distance in cm, adjust as needed
...
void loop() {
  ...
  if (objectDetected) {
    ...
    double proportional = kpwall * error;
    kiTotal += error;
    double integral = kiwall * kiTotal;
    if(kiTotal > 1000) {
      kiTotal = 1000;
    }
    ...
    float derivative = kdwall * (error - previousError);
    previousError = error;
